...
本文档为**微信小程序/小游戏**接入 [turbo 引力引擎](https://gravity-engine.com/)的技术接入方案，具体Demo请参考[GitHub开源项目](https://github.com/GravityInfinite/Turbo-MiniProgram-Demo)。

#### 引入

下载`turbo.min.js`文件并引入到工程中
 ...
 * @param {string} media_type 投放的媒体类型
    ...
    kuaishou 快手广告
    tencent 腾讯广告
 * @param {string} wx_openid 用户wxopenid 微信小程序必填
 * @param {string} wx_unionid 微信union_id 微信小程序选填
 */
...
/**
 * 用户数据更新
 * @param {string} click_id 用户点击广告id 微信小程序传入选填
 * @param {string} wx_openid 用户wxopenid 微信小程序传入
 * @param {string} wx_unionid 微信union_id 微信小程序选填
 * @param {string} media_type 投放的媒体类型
    bytedance 头条（巨量广告）
    kuaishou 快手广告
